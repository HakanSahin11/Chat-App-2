...
namespace Chat_App.Helper_Classes
{
    public class CryptoGraphy : IDisposable
    {
        readonly Aes _algorithm;
        readonly byte[] _salt;

        //Standard encryption / decryption key meant for endpoint connection via API to API security connection
        //Key conditions are that there's been sent no salt info either from the DB or generate hash HashSalt method
    ...
    public class Salting
    {
        ...
        public static SaltModel HashSalt(string pass, byte[] saltBytes)
        {
            try
            {
                if (saltBytes == null)
                ...
                return new SaltModel(Convert.ToBase64String(ResultHashSalt), Convert.ToBase64String(saltBytes));
            }
            catch
            {
